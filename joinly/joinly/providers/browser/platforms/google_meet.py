...
class GoogleMeetBrowserPlatformController(BaseBrowserPlatformController):
    ...
    async def join(
        ...
        await join_btn.wait_for(state="visible", timeout=20000)
        # Signed-in Google users have no guest-name field.
        if await name_field.is_visible():
            await name_field.fill(name, timeout=20000)
        await join_btn.click(timeout=1000)

        # Local demo diagnostics: record the page the join action actually reached.
        await page.locator("body").filter(has_text=re.compile(r"\S")).wait_for(timeout=20000)
        page_text = await page.locator("body").inner_text()
        if "you can't join this video call" in page_text.lower():
            raise ValueError("Google Meet rejected guest admission: You can't join this video call.")

        if not await self._check_joined(page, timeout=120):
